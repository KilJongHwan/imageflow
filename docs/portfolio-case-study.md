...
- health endpoint에 `processingMode`, `queueDepth`, `storageProvider`, `uploadRequestsPerMinute`를 포함해 운영 상태를 바로 볼 수 있게 했습니다.
...
- 배치 10개 제한, ZIP 32엔트리/20MB/50MB 제한, 30rpm rate limit, 3회 retry를 적용해 대용량 처리 리스크를 줄이는 방향으로 하드닝
