...
`대용량 상품 이미지 업로드로 인해 발생하는 트래픽 및 스토리지 비용 증가 문제를 완화하기 위한 이미지 최적화 워크스페이스 개발`

## 프로젝트 개요

ImageFlow는 커머스 환경에서 반복적으로 발생하는 상품 이미지 업로드, 크롭, 리사이즈, 압축, 다운로드 작업을 하나의 흐름으로 정리한 프로젝트입니다.  
단순 이미지 편집기가 아니라, `운영 비용 절감`과 `실무 작업 효율화`를 함께 다루는 서비스형 워크스페이스를 목표로 설계했습니다.
...
- 현재 배포 데모 모드에서는 Spring Boot 동기 처리로도 동작하지만,
- 이미지 처리량이 커질 경우 `API 서버`와 `이미지 처리 워커`를 분리할 수 있도록 확장 구조를 먼저 고려함
...
### 1. 단순 편집기가 아니라 문제 해결형 서비스로 접근

처음에는 이미지 최적화 기능 자체에 집중했지만, 프로젝트 방향을 다음처럼 다시 정의했습니다.

- 무엇을 만들었는가: 이미지 최적화 툴
- 무엇을 해결하는가: 대용량 상품 이미지 업로드로 인한 비용과 반복 운영 문제

이렇게 문제 중심으로 재정의하면서 포트폴리오 설득력이 훨씬 좋아졌습니다.
...
### 5. 데모 모드와 지원 기능의 경계를 명확히 한 점

초기에는 동기 처리 모드와 비동기 생성 경로가 함께 존재해, 내부적으로는 지원하지 않는 흐름이 조용히 남아 있는 문제가 있었습니다.
이를 정리하면서:

- 데모용 sync 모드에서는 업로드 기반 최적화 흐름만 지원
- 프롬프트 기반 생성 경로는 명시적으로 차단
- 프론트에서는 로그아웃 시 polling 요청을 정리

하도록 바꾸었습니다. 이 작업은 기능 추가보다도 “어떤 모드에서 무엇이 실제로 지원되는가”를 분명하게 만드는 안정화 작업이었습니다.
...
- sync 데모 모드에서 지원 범위를 명확히 제한하고, 세션 종료 시 불필요한 polling 요청을 끊어 상태 일관성을 개선

## 아쉬운 점 / 다음 단계

- 마켓별 preset을 더 세분화할 필요가 있음
- 최근 작업 히스토리와 상태 필터링이 더 필요함
- 이미지 비교 뷰를 더 정교하게 다듬을 필요가 있음
- 현재 데모 배포는 동기 처리 기반이라, 실서비스라면 비동기 처리로 옮겨야 함
- 로컬 저장 대신 Cloudflare R2 / S3 중심 구조로 전환할 필요가 있음
- 대량 처리 환경에서는 rate limit, retry, observability가 추가되어야 함
...
- JWT 인증, 다중 이미지 업로드, ZIP 업로드, 수동 크롭, 결과 ZIP 다운로드 기능을 구현했습니다.
- 원본 대비 결과 파일 크기, 절감량, 절감률을 함께 제공해 비용 관점에서 최적화 효과를 설명할 수 있도록 설계했습니다.
- 데모 배포를 고려해 동기 처리 기반 구조를 우선 적용하고, Redis Queue + Python worker 기반 비동기 확장 구조를 함께 설계했습니다.
