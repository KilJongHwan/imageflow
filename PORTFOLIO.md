...
- 현재 로컬 기준으로는 `32개 엔트리`, `50MB per entry`, `200MB total extracted size`를 넘기면 즉시 거절합니다.
- 프런트에서는 이 예외를 그대로 노출하지 않고, “ZIP 안의 개별 이미지가 너무 큽니다”, “ZIP 전체 용량이 너무 큽니다”처럼 사용자가 이해하기 쉬운 문구로 바꿨습니다.
...
### 5. 배치 작업은 비동기였지만 실제 처리는 순차적이라 결과가 한참 대기하던 문제

문제:

- ZIP 안에 여러 이미지가 있어도, worker가 한 번에 하나씩만 처리해 첫 작업이 끝날 때까지 나머지 작업이 오래 `QUEUED` 상태로 남았습니다.

원인 분석:

- Redis queue 기반 비동기 구조는 있었지만, Python worker가 `BLPOP`으로 job을 하나씩 꺼내 순차 처리하는 구조였습니다.
- 프런트도 배치 진행 상태를 계속 따라가지 않아 사용자는 “한 개만 완료되고 나머지는 멈춘 것 같다”고 느끼기 쉬웠습니다.

해결:

- worker에 `WORKER_CONCURRENCY`를 도입해 기본 `3개` 작업까지 동시 처리할 수 있게 변경했습니다.
- 프런트에는 배치 단위 polling을 추가해 `3개 중 1개 완료, 2개 처리 중`처럼 실제 진행 상황이 바로 보이도록 수정했습니다.

왜 이 방식이 맞았는가:

- 비동기 구조만으로는 체감 성능이 충분하지 않았고, 배치 업로드를 “실제로 쓸 수 있는 수준”으로 끌어올리려면 worker 동시성과 진행 가시성이 함께 필요했습니다.
...
### 6. 병렬 worker가 너무 빨리 콜백을 보내며 job not found를 만들던 문제

문제:

- worker 동시성을 높인 뒤 일부 job이 `PATCH /api/image-jobs/{id}/result` 단계에서 `404`로 실패했습니다.

원인 분석:

- image job 저장 로직은 트랜잭션 안에서 수행되는데, queue publish도 같은 흐름 안에서 즉시 발생했습니다.
- 병렬 worker가 빨라지자 DB 커밋 전에 콜백이 먼저 도착하는 경쟁 상태가 생겼습니다.

해결:

- queue publish를 즉시 수행하지 않고, Spring 트랜잭션의 `afterCommit` 이후에만 Redis로 전송하도록 변경했습니다.

왜 이 방식이 맞았는가:

- 비동기 시스템에서는 “메시지를 보냈다”보다 “소비자가 읽을 수 있는 상태가 먼저 준비됐는가”가 더 중요합니다.
- 이 수정으로 병렬 worker 환경에서도 job lifecycle 순서를 안정적으로 맞출 수 있었습니다.
...
- ZIP 배치 업로드를 위한 worker 동시 처리와 트랜잭션 커밋 이후 queue publish 보강
...
- ZIP 업로드는 최대 `32개 엔트리`, 엔트리당 최대 `50MB`, 전체 압축 해제 기준 최대 `200MB`로 제한했습니다.
...
- queue backlog가 `100` 이상 쌓이면 새 업로드를 제한하는 backpressure를 넣어, 처리보다 입력이 더 빠를 때 시스템이 무한정 밀리지 않도록 했습니다.
- health 응답에는 `processing mode`, `queue enabled`, `queue depth`, `maxQueueBacklogDepth`, `queueWritable`, `storage provider`, `upload rate limit`를 포함해 운영 상태를 바로 확인할 수 있게 했습니다.
...
- 대량 업로드 리스크를 줄이기 위해 배치 10개 제한, ZIP 32엔트리/50MB/200MB 제한, 스트리밍 ZIP 다운로드를 적용해 메모리 및 압축 해제 부담을 제어했습니다.
- Redis queue + Python worker 기반 비동기 처리 구조, 3회 retry, 분당 30회 업로드 rate limit, health 기반 운영 상태 노출을 통해 데모 수준을 넘는 운영형 구조를 정리했습니다.
- backlog 상한을 `100`으로 두고 queue depth 기준 backpressure를 추가해, worker 처리 속도를 넘는 입력이 들어올 때도 무제한 적재를 피하도록 설계했습니다.
- worker 동시 처리(`concurrency=3`)와 트랜잭션 `afterCommit` 이후 queue publish를 적용해, ZIP 배치 업로드가 실제로 안정적으로 병렬 처리되도록 보완했습니다.
...
- 대용량 처리 리스크는 `10개 배치`, `32엔트리`, `50MB`, `200MB`, `30rpm`, `3회 retry`, `concurrency=3`로 설명
