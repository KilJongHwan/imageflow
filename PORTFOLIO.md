...
- 커머스 상품 이미지 업로드 과정에서 발생하는 전송량·저장소 비용 문제를 줄이기 위해, React·Spring Boot·Python 기반 이미지 최적화 워크스페이스를 설계하고 구현했습니다.
- 배치 업로드, ZIP 업로드, 수동 크롭, 선택적 워터마크, 결과 ZIP 다운로드를 포함한 셀러용 이미지 처리 흐름을 구축했습니다.
- ZIP 32엔트리/50MB/200MB 제한, upload rate limit, queue retry, backlog 상한을 적용해 대용량 업로드 리스크를 통제했습니다.
- worker concurrency와 `afterCommit` 이후 queue publish를 적용해, 비동기 배치 처리에서 발생한 순차 병목과 result callback race condition을 해결했습니다.
