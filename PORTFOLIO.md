...
이 문제를 줄이기 위해 worker에 `WORKER_CONCURRENCY`를 넣어 기본 3개 작업까지 동시에 처리하도록 바꿨고, 프론트도 배치 진행 상태를 계속 따라가도록 수정했습니다.
...
### 5. 병렬 worker 도입 후 생긴 race condition을 DB Outbox 구조로 정리

worker를 빠르게 병렬 처리로 바꾸자, 일부 작업이 결과 콜백 단계에서 `404`로 실패했습니다.  
원인을 확인해 보니, job 저장 트랜잭션이 커밋되기 전에 worker가 결과를 먼저 보내는 경쟁 상태가 있었습니다.

처음에는 queue publish 시점을 `afterCommit` 이후로 미뤄 문제 원인을 확인했고, 이후에는 메시지를 Redis로 바로 보내지 않고 DB Outbox 테이블에 먼저 적재한 뒤 relay가 전송하도록 구조를 바꿨습니다.  
이 수정으로 `저장 -> 커밋 -> outbox 적재 -> relay 전송 -> worker 처리` 순서를 보장할 수 있었습니다.
...
- [ImageJobOutboxRelay.java](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/backend/src/main/java/com/imageflow/backend/queue/ImageJobOutboxRelay.java:1)
...
### 7. 분리 배포된 backend와 worker가 파일 시스템을 공유하지 않는 문제

Render처럼 backend와 worker가 서로 다른 서비스로 올라가면, local storage 경로를 직접 공유할 수 없습니다.  
이 상태에서 worker가 결과 파일을 자기 컨테이너에만 저장하면, 백엔드는 성공 상태를 알아도 다운로드 파일을 제공할 수 없습니다.

이 문제를 해결하기 위해:

- worker가 `result-file` callback endpoint로 최적화 결과 파일 자체를 다시 업로드
- backend는 callback으로 받은 파일을 자신의 output 경로에 저장
- Render async 배포 예시에서는 `REDIS_URL`, `BACKEND_HOSTPORT`, `WORKER_RESULT_CALLBACK_ENABLED`를 사용
...
- [ImageJobService.java](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/backend/src/main/java/com/imageflow/backend/domain/image/ImageJobService.java:1)
- [worker.py](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/image-agent/worker.py:1)
- [render.async.yaml](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/render.async.yaml:1)

## 운영 관점에서 보강한 부분

- upload rate limit 적용
- queue publish retry 적용
- DB Outbox 기반 메시지 적재 및 relay 적용
- queue backlog 상한 도입
- health endpoint에서 `processingMode`, `queueDepth`, `queueWritable`, `storageProvider` 노출
- health endpoint에서 `outboxPendingCount` 노출
- 로그아웃 시 polling 정리
...
- API 문서: Swagger UI(`/swagger-ui`), OpenAPI JSON(`/api-docs`), [api-reference.md](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/docs/api-reference.md:1)
- Docker 실행: `docker compose` 기반으로 `postgres`, `redis`, `image-agent` 구동 가능
- 부하테스트: [concurrent_upload_test.py](/abs/path/c:/Users/tsline/IdeaProjects/imageflow/load-tests/concurrent_upload_test.py:1) 추가
- 장애 대응 구조: rate limit, queue retry, backlog control, DB Outbox relay, health endpoint 반영
- Render async 예시: `render.async.yaml` 기준 backend + keyvalue + worker 분리 가능

현재 저장소 기준으로 명확하게 확인되는 항목은 다음과 같습니다.

- Docker 실행은 `docker-compose.yml` 기준으로 `postgres`, `redis`, `image-agent` 구동이 가능하도록 구성되어 있습니다.
- 장애 대응 구조는 upload rate limit, queue retry, backlog control, health endpoint, worker concurrency, DB Outbox relay 형태로 반영되어 있습니다.
- 분리 배포 환경에서는 worker result-file callback으로 shared storage 없이 결과 파일을 백엔드에 반영할 수 있습니다.

## 주요 성과

- 업로드, 배치 처리, 결과 다운로드를 하나의 흐름으로 묶은 이미지 처리 워크스페이스를 구현했습니다.

- Redis 기반 비동기 worker 구조를 적용하고, worker concurrency와 queue backlog 제어를 반영해 배치 업로드 처리 흐름을 안정화했습니다.

- 병렬 worker 도입 과정에서 발생한 result callback race condition을 추적했고, 최종적으로 DB Outbox relay 구조로 queue publish 신뢰성을 보강했습니다.

- ZIP 업로드 제한, 스트리밍 ZIP 다운로드, health 노출, API 문서, Docker 실행 구성을 통해 데모 수준을 넘는 운영형 구조로 정리했습니다.

- 분리 배포 환경에서 shared filesystem이 없다는 제약을 result-file callback 구조로 풀어, Render async 배포까지 이어질 수 있는 형태로 정리했습니다.
